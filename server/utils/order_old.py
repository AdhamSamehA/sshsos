...
async def aggregate_shared_cart_items(shared_cart):
    """
...
async def create_order_from_shared_cart(db: AsyncSession, shared_cart):
    """
    Create an order for the shared cart, aggregating items.
    """
    # Aggregate items by item_id
    aggregated_items = await aggregate_shared_cart_items(shared_cart)

    # Create the order
    order = Order(
        user_id=shared_cart.organizer_id,
        shared_cart_id=shared_cart.id,
        total_amount=sum(item["total_price"] for item in aggregated_items.values()) + shared_cart.supermarket.delivery_fee,
        status=OrderStatus.COMPLETED,
    )
    db.add(order)
    await db.flush()
